...
@section('content')
<div class="container">
    <h5> <a href='/cooking/record/'>レシピ記録</a></h5>
    ...
    <form class="meal-input-form" method="post" action="/cooking/record/store_recipe" autocomplete="off">
      @csrf
      <input type="hidden" name="recipe_id" value="{{$recipe_id}}">
      <h6>菜種指定</h6>
      <div class="meal-select">
        @foreach ($meal_kind as $row_record)
          @if ($row_record->kind_id === 1)
              <?php $defalut_value = 'checked'; ?>
          @else
              <?php $defalut_value = ''; ?>
          @endif
          <input id="select{{ $row_record->kind_id }}" class="radio-inline__input" type="radio" name="kind_id"
                 value="{{ $row_record->kind_id }}" {{$defalut_value}}>
          <label class="radio-inline__label" for="select{{ $row_record->kind_id }}">{{ $row_record->kind_nm }}</label>
        @endforeach
